...
module Plutus.Contracts.DropSpotTradeSM (
      -- tsSBS, tsSerialised,
      TokenSale (..)
    , TSRedeemer (..)
    , TSStartSchema
...
import           PlutusTx.Prelude             hiding (Semigroup(..), check, unless, uncurry)
...
data TokenSale = TokenSale {
    tsSeller :: !PubKeyHash,          -- DS Payback address? Commission
    tsToken  :: !AssetClass,          -- Needs to be Datum - I.E., The token that we want to trade
    tsTT     :: !(Maybe ThreadToken)  -- Needs to be Datum
} deriving (Show, Generic, FromJSON, ToJSON, Prelude.Eq)

PlutusTx.makeLift ''TokenSale

data TSRedeemer = SetPrice Integer | AddTokens Integer | BuyTokens Integer | Withdraw Integer Integer | Close
...
-- tsValidator :: Validator
-- tsValidator = Scripts.validatorScript tsTypedValidator

-- tsAddress :: Ledger.Address
-- tsAddress = scriptAddress tsValidator



-- tsSBS :: SBS.ShortByteString
-- tsSBS = SBS.toShort . LBS.toStrict $ serialise tsValidator

-- tsSerialised :: PlutusScript PlutusScriptV1
-- tsSerialised = PlutusScriptSerialised tsSBS
...
-- mkSchemaDefinitions ''TSStartSchema
-- mkSchemaDefinitions ''TSUseSchema
